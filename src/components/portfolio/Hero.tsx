...
export function Hero() {
  return (
    <section id="top" className="relative min-h-screen flex items-center justify-center px-6 pt-32 pb-20 overflow-hidden">
      ...
      {/* PERFORMANCE OVERRIDE VIA CSS:
        - `md:animate-float` ensures floating animations run strictly on laptops and desktops.
        - On mobile devices, the animations are completely static, keeping your laptop cursor glow perfectly active!
      */}
      <div className="absolute top-1/4 -left-20 w-96 h-96 rounded-full bg-[var(--neon)]/20 blur-[120px] md:animate-float" />
      <div className="absolute bottom-1/4 -right-20 w-96 h-96 rounded-full bg-[var(--purple)]/20 blur-[120px] md:animate-float" style={{ animationDelay: "1.5s" }} />

      <div className="relative max-w-7xl w-full grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center transform-gpu">
        <div>
          <motion.div 
          ...
          >
            ...
            <span className="neon-text animate-text-glow inline-block">digital experiences</span><br />
            for the future.
          </motion.h1>
          ...
          >
            <span className="text-[var(--neon)]">$</span> <TypewriterText /><span className="animate-blink text-[var(--cyan)]">▍</span>
          </motion.div>
          ...
          >
            <a href="#projects" className="group relative inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold bg-gradient-to-r from-[var(--neon)] to-[var(--purple)] text-background overflow-hidden transform-gpu">
        ...
        <motion.div 
          ...
          className="relative mx-auto w-full max-w-md aspect-[4/5] transform-gpu"
        >
          {/* Pulsing neon loop restricted cleanly to desktop viewports with `md:animate-pulse-glow` */}
          <div className="absolute -inset-4 rounded-3xl bg-gradient-to-br from-[var(--neon)] via-[var(--purple)] to-[var(--cyan)] opacity-60 blur-2xl md:animate-pulse-glow" />
          ...
          {/* Left/Right Floating Technology Badges using pure CSS media selectors */}
          <div className="absolute -bottom-6 -left-6 glass-strong rounded-2xl px-4 py-3 font-mono text-xs border border-white/5 md:animate-float" style={{ animationDelay: "0.5s" }}>
            <div className="text-[var(--neon)]">{"<SpringBoot/>"}</div>
          </div>
          <div className="absolute -top-6 -right-6 glass-strong rounded-2xl px-4 py-3 font-mono text-xs border border-white/5 md:animate-float">
